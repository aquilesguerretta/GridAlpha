  ...
  useEffect(() => {
    const fetchBatteryData = async () => {
      try {
        ...
        if (zoneRecord) {
          // Transform API data into 24-hour schedule
          ...
          for (let hour = 0; hour < 24; hour++) {
            ...
            // Check if hour is in charge_hours array
            if (zoneRecord.charge_hours.includes(hourStr)) {
              action = 'charge';
              price = zoneRecord.charge_price;
            }
            // Check if hour is in discharge_hours array
            else if (zoneRecord.discharge_hours.includes(hourStr)) {
              action = 'discharge';
        ...
        <Card className="p-6 bg-card border-border backdrop-blur-sm">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Slider
                value={[cyclingCost]}
                onValueChange={(value) => setCyclingCost(value[0])}
                min={0}
                max={150}
                step={5}
                className="w-full"
